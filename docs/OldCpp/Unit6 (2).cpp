...
range=atof(OKBottomDlg->Edit9->Text.c_str());
 freq=atof(Form5->Edit1->Text.c_str());
Form5->Show();
  SDIAppForm->Chart1->BottomAxis->Automatic=false;
  wdt=20*(atof(OKBottomDlg->Edit14->Text.c_str())+atof(OKBottomDlg->Edit15->Text.c_str()));
...
{
SDIAppForm->OpenDialog1->Title="Delete Template";
SDIAppForm->OpenDialog1->Filter = "Templates|*.DB";
 SDIAppForm->OpenDialog1->FileName = "*.DB";
 SDIAppForm->OpenDialog1->InitialDir=ExtractFilePath(Application->ExeName)+"Templates";
  ...
  if (SDIAppForm->OpenDialog1->Execute())
  {
  if(DeleteFile(SDIAppForm->OpenDialog1->FileName))
  ShowMessage("Template deleted");
  else
  ShowMessage("Template not deleted");
 }
 SDIAppForm->OpenDialog->InitialDir=SDIAppForm->Edit14->Text;
}
//---------------------------------------------------------------------------
                        ...
                        aaa:
                         ...
                         float sac=0;
                           for (int z=1; z < Form23->StringGrid1->RowCount;z++)
                             {
                                if (fabs(atof(Form23->StringGrid1->Cells[2][z].c_str())-asswave)<0.04)
                                {
                                 sac=atof(Form23->StringGrid1->Cells[3][z].c_str());
                                 break;
                                }
                                }
                              if (sac==0) sac=1;
                 ...
                 if (lin1 !=0)
                     {
                       ...
                       inte1/=(yyy*sac);
                        ...
                        float de=atof((Form27->Edit8->Text).c_str());
                        float tem=atof((Form27->Edit7->Text).c_str());
