...
if (StrLen(Form2->Edit1->Text.c_str()) == 1)
{
...
else
{
 ...
 sprintf(ss,"%s%s",UpperCase(Form2->Edit3->Text.c_str()),Form2->Edit4->Text.c_str());
 Form2->Edit1->Text=ss;
...
{
case 0:
sprintf(sqls,"SELECT * FROM datalibs WHERE ([specie] = '%s' OR [specie] = '%s')and [wavelen] > %f and [wavelen] < %f and [ion]=1 order by -[inte]",Form2->Edit1->Text.c_str(),spi.c_str(),SDIAppForm->Chart1->BottomAxis->Minimum,SDIAppForm->Chart1->BottomAxis->Maximum);
 ...
 rc=Form5->Query1->RecordCount;
  if (rc > atof(Form2->Edit2->Text.c_str()))
  rc= atof(Form2->Edit2->Text.c_str());
 Form5->Query1->First();
...
break;
case 1:
sprintf(sqls,"SELECT * FROM datalibs WHERE ([specie] = '%s' OR [specie] = '%s') and [wavelen] > %f and [wavelen] < %f and [ion]=2 order by -[inte]",Form2->Edit1->Text.c_str(),spi.c_str(),SDIAppForm->Chart1->BottomAxis->Minimum,SDIAppForm->Chart1->BottomAxis->Maximum);
 Form5->Query1->SQL->Clear();
 Form5->Query1->SQL->Add(sqls);
 Form5->Query1->Open();
 if (Form5->Query1->RecordCount == 0)
...
break;
case 2:
sprintf(sqls,"SELECT * FROM datalibs WHERE ([specie] = '%s' OR [specie] = '%s') and [wavelen] > %f and [wavelen] < %f order by -[inte]",Form2->Edit1->Text.c_str(),spi.c_str(),SDIAppForm->Chart1->BottomAxis->Minimum,SDIAppForm->Chart1->BottomAxis->Maximum);
 Form5->Query1->SQL->Clear();
 Form5->Query1->SQL->Add(sqls);
 Form5->Query1->Open();
 if (Form5->Query1->RecordCount == 0)
