...
double tmp=atof(AnsiString(Form9->Edit7->Text).c_str())*11600.0;
double denel=atof(AnsiString(Form9->Edit8->Text).c_str());
float aut;

 if (atof(AnsiString(Form23->StringGrid1->Cells[3][progre]).c_str()) != 0.0)
  aut=atof(AnsiString(Form23->StringGrid1->Cells[3][progre]).c_str());
  else
  aut=1.0;
...
if (OKBottomDlg->CheckBox9->Checked) {
 inst=atof(AnsiString(OKBottomDlg->Edit16->Text).c_str())/5000.*wave;
}
...
// lor*=pow(aut,1.152);
 ...
 float perc;
   double zi= zZ(AnsiString(Form9->Edit3->Text).c_str(),1,atof(AnsiString(Form9->Edit7->Text).c_str()));
   double zii= zZ(AnsiString(Form9->Edit3->Text).c_str(),2,atof(AnsiString(Form9->Edit7->Text).c_str()));
 ...
 {
 perc=atof(AnsiString(Form9->StringGrid2->Cells[2][w]).c_str());
 break;
 }
 }
...
double Nl=nq*atof(AnsiString(Form21->Edit14->Text).c_str())*perc/100.0;
...
// double integ=0.0;
// double inte0=0.0;
double lu=atof(AnsiString(Form21->Edit13->Text).c_str());
/* double knu,tt,X;
