...
do {
     do   {
      replot();            Form27->Refresh();
		...
		oldtemp=atof(AnsiString(Form27->Edit7->Text).c_str());
        float j=0;
        float tmed=0;
        for (int i=1;i< Form27->StringGrid2->RowCount;i++)
        {
		float ff=atof(AnsiString(Form27->StringGrid2->Cells[3][i]).c_str());
		...
		float Neold=atof(AnsiString(Form27->Edit8->Text).c_str());
		...
		Form27->Edit8->Text=FormatFloat("0.00E+00",Ne(Neold,11600*newtemp,atof(AnsiString(Form16->Edit3->Text).c_str())));
	   ...
	   contr=fabs(atof(AnsiString(Form27->Edit8->Text).c_str())-Neold)/Neold;
    } while (contr > 0.01);
 ...
 for (int ii=0;ii < sc;ii++)
   {
        ...
        MyLinSeries->SeriesColor=Form27->Chart1->Series[ii]->SeriesColor;
		 if ((-1/ktt > atof(AnsiString(OKBottomDlg->Edit20->Text).c_str())) & (-1/ktt < atof(AnsiString(OKBottomDlg->Edit21->Text).c_str())))
        {
...
{
if (atof(AnsiString(Form27->Edit8->Text).c_str())==0) return;
void showsahaboltz();
showsahaboltz();
}
//---------------------------------------------------------------------------
