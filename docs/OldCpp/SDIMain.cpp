...
Chart1->Visible=true;
 ...
 Chart1->BottomAxis->Automatic=true;
  ...
  handle = open(AnsiString(OpenDialog->FileName).c_str(),O_RDONLY);
...
apa:
  ...
  if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str())) & (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str())))
...
void __fastcall TSDIAppForm::FileSave1Execute(TObject *Sender)
{
  ...
  FILE *out;
   ...
   out = fopen(AnsiString(SaveDialog->FileName).c_str(), "w");
...
while(!feof(strm));
  ...
  handle=open(AnsiString(Memo1->Lines->Strings[j]).c_str(),O_RDONLY);
  stream = fdopen(handle, "r");
  int ndata=0;
   Series1->XValues->Order = loAscending;
 do
 ...
 fscanf(stream,"%le %le", &wave, &lips);
  ...
  if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500))
	...
	Series1->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ',clBlack);
   }
   while (!feof(stream));
   close(handle);
...
while(!feof(strm));
  ...
  handle=open(AnsiString(Memo1->Lines->Strings[j]).c_str(),O_RDONLY|O_BINARY);
      heade=malloc(1539);
   ...
   for (w=0;w<fin;w++)
   {
    ...
    if (OKBottomDlg->CheckBox1->Checked==true)
    {
      float norma=Form12->Series1->YValues->Value[ndata];
       ...
       x=dt[0]+nn*dt[1];
			 Series1->AddXY(x+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),y,' ',clBlack);
      nn++;
      if (nn==(dt[2]-1))
      {
      flag=1;
...
{
Lambda_cursor->Caption="0000.00";
Intensity_cursor->Caption="0000.00";
}
else
{
xx=X;
yy=Y;
fattx=((xx-Chart1->BottomAxis->IStartPos)/Chart1->BottomAxis->IAxisSize);
fatty=((yy-Chart1->LeftAxis->IEndPos)/Chart1->LeftAxis->IAxisSize);
...
Lambda_cursor->Caption=FormatFloat("#0000.00",capx);
Intensity_cursor->Caption=FormatFloat("#0000.00",capy);
}
}
//---------------------------------------------------------------------------
...
if (Button==mbLeft && Shift.Contains(ssAlt) && Shift.Contains(ssCtrl))
 {
 Form4->Show();
 Form4->Edit1->Text=Intensity_cursor->Caption.c_str();
 return;
 }
 else if (Button==mbRight && Shift.Contains(ssAlt)&& Shift.Contains(ssCtrl))
 {
 Form4->Show();
    Form4->Edit2->Text=Intensity_cursor->Caption.c_str();
 return;
 }
 else if (Button==mbLeft && Shift.Contains(ssShift) && Shift.Contains(ssCtrl))
 {
 SDIAppForm->Series2->AddXY(atof(AnsiString(SDIAppForm->Lambda_cursor->Caption).c_str()),atof(AnsiString(SDIAppForm->Intensity_cursor->Caption).c_str()),' ',clRed);
 SDIAppForm->Series2->XValues->Order = loAscending;
  SDIAppForm->Series2->Active=true;
 return;
 }
 else if (Button==mbRight && Shift.Contains(ssShift)&& Shift.Contains(ssCtrl))
 {
 SDIAppForm->Series2->Active=true;
 return;
 }
 ...
 {
 float xx=atof(AnsiString(Lambda_cursor->Caption).c_str());
identif(xx);
}
...
range=atof(AnsiString(OKBottomDlg->Edit9->Text).c_str());
 freq=atof(AnsiString(Lambda_cursor->Caption).c_str());
  Form5->Edit1->Text=AnsiString(Lambda_cursor->Caption).c_str();
sprintf(sqls,"SELECT * FROM daticerti WHERE [wavelen] BETWEEN %f AND %f ORDER BY abs([wavelen]-%f)",(freq-range),(freq+range),freq);
 ...
 {
 Form11->Show();
 Form11->Edit1->Text=Lambda_cursor->Caption.c_str();
 }
 else if (Button==mbRight && Shift.Contains(ssAlt))
 {
 Form11->Show();
    Form11->Edit2->Text=Lambda_cursor->Caption.c_str();
 }
}
//---------------------------------------------------------------------------
...
void __fastcall TSDIAppForm::Resample1Click(TObject *Sender)
{
  ...
  int oldj=0;
  float xx,yy;
  xx=atof(AnsiString(OKBottomDlg->Edit3->Text).c_str());
...
void loadtempl(AnsiString nome);
 ...
 SDIAppForm->OpenDialog1->InitialDir="C:\\LIBS++\\Templates";
...
void __fastcall TSDIAppForm::Full_XClick(TObject *Sender)
{
 double Max,Min;
 Max=Chart1->BottomAxis->Maximum;
 Min=Chart1->BottomAxis->Minimum;
 Chart1->BottomAxis->SetMinMax(Min-(Max-Min)/3, Max+(Max-Min)/3);

}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::Full_YClick(TObject *Sender)
{
	 float minay=0;
	 float maxay=0;
...
void __fastcall TSDIAppForm::Full_ScaleClick(TObject *Sender)
{
 Chart1->BottomAxis->Automatic=true;
  Chart1->LeftAxis->Automatic=true;
}
...
void __fastcall TSDIAppForm::Copy1Click(TObject *Sender)
{
...
void __fastcall TSDIAppForm::HelpAboutItemClick(TObject *Sender)
{
  ...
  if (VerQueryValue(pBuf, temp.c_str(), (LPVOID *)&pValue, &Len))
        ...
        {
		AboutBox->Version->Caption= "Version 5.0" ;
        }
    free(pBuf);
  }
...
TVarRec vrs[1];
 ...
 {
    spe="I";
	 io=AnsiString("I").c_str();
 }
 else
 {
 spe=ppp.SubString(1,lg-2);
 io=AnsiString("II").c_str();
 }
 sprintf(la,"%s%s",AnsiString(Trim(spe)).c_str(),AnsiString(Trim(io)).c_str());
 fprintf(out,"%f %f %s \n", Series1->XValues->Value[w], Series1->YValues->Value[w],la);
       i++;
 }
 else
 {
fprintf(out,"%f %f %s \n", Series1->XValues->Value[w], Series1->YValues->Value[w],AnsiString(Series4->XLabel[i]).c_str());
       i++;
 }
 }
 else
...
if (strcmp(pp,"none")== 0) {OKBottomDlg->Edit13->Text="";}
 ...
 OKBottomDlg->Edit3->Text=fl;
  ...
  OKBottomDlg->Edit4->Text=fl;
   ...
   fscanf(out,"%i // Ha Lines\n", &i);
   OKBottomDlg->Edit1->Text=i;

   fscanf(out,"%f // Ha Range\n", &fl);
   OKBottomDlg->Edit2->Text=fl;

      fscanf(out,"%f // First wavelength\n", &fl);
   OKBottomDlg->Edit5->Text=fl;

      fscanf(out,"%f // Second wavelength\n", &fl);
   OKBottomDlg->Edit6->Text=fl;

      fscanf(out,"%f // First width\n", &fl);
   OKBottomDlg->Edit12->Text=fl;

      fscanf(out,"%f // Second width\n", &fl);
   OKBottomDlg->Edit24->Text=fl;

      fscanf(out,"%f // KappaT\n", &fl);
   OKBottomDlg->Edit25->Text=fl;
...
if (strcmp(pp,"none")== 0) {OKBottomDlg->Edit10->Text="";}
 ...
 //Windows positions
          ...
          fscanf(out,"%i %i %i %i // Main\n", &i1,&i2,&i3,&i4);
          SDIAppForm->Top=i1;
          SDIAppForm->Left=i2;
          SDIAppForm->Height=i3;
          SDIAppForm->Width=i4;

          fscanf(out,"%i %i %i %i // Template\n", &i1,&i2,&i3,&i4);
          Form6->Top=i1;
          Form6->Left=i2;
          Form6->Height=i3;
          Form6->Width=i4;
          ...
          fscanf(out,"%i %i // Options\n", &i1,&i2);
          OKBottomDlg->Top=i1;
          OKBottomDlg->Left=i2;

          fscanf(out,"%i %i // Ident\n", &i1,&i2);
          Form5->Top=i1;
          Form5->Left=i2;

          fscanf(out,"%s // InDir\n", &pp);
		  SDIAppForm->Edit14->Text=pp;
...
//---------------------------------------------------------------------------
 ...
 {
  float norma=1;
   if (SDIAppForm->Series1->MaxYValue() > 0) norma=SDIAppForm->Series1->MaxYValue();
 ...
 float maxim=Form6->Query1->FieldValues["templint"];
	 float fatt=norma/maxim;
sprintf(sqls,"SELECT * FROM '%s' ORDER BY WAVELEN",AnsiString(Form6->Table1->TableName).c_str());
 Form6->Query1->SQL->Clear();
 Form6->Query1->SQL->Add(sqls);
 Form6->Query1->Open();
 Form6->Query1->First();
 Form6->Caption="Active Template:"+nome;
  ...
  int ioniz=Form6->Table1->FieldByName("ion")->AsInteger;
    if (ioniz != 0)
	{
	AnsiString nspc= Form6->Table1->FieldByName("specie")->AsString;
  switch(ioniz)
  {
  case 1:
  sprintf(lab,"%s I",nspc);
   break;
  case 2:
	 sprintf(lab,"%s II",nspc);
  break;
  }
  if (Form6->Table1->FieldByName("Aki")->AsFloat > soglia)
  {
   SDIAppForm->Series4->AddXY(Form6->Table1->FieldValues["wavelen"],Form6->Table1->FieldValues["templint"]*fatt,lab,clYellow);
   }
   else
   {
    SDIAppForm->Series4->AddXY(Form6->Table1->FieldValues["wavelen"],Form6->Table1->FieldValues["templint"]*fatt,lab,COLORE);
   }
    }
     else
    {
   sprintf(lab,"%4.2f",Form6->Table1->FieldByName("wavelen")->AsFloat);
   SDIAppForm->Series4->AddXY(Form6->Table1->FieldValues["wavelen"],Form6->Table1->FieldValues["templint"]*fatt,lab,clBlue);
   }
 Form6->Table1->Next();
  }
...
//  SDIAppForm->Series5->Clear();
 ...
 SDIAppForm->Chart1->BottomAxis->Automatic=true;
  ...
  SDIAppForm->StatusBar->SimpleText="LIBS++ Software for Spectral Analysis (c) 1999-2014 CNR Applied Laser Spectroscopy Laboratory";
  ...
  handle = open((nomefile).c_str(),O_RDONLY);
   SDIAppForm->Series1->XValues->Order = loAscending;
	 ...
	 int trt, roh;
	   trt=AnsiPos(".TRT",AnsiUpperCase(nomefile));
	   roh=AnsiPos(".ROH",AnsiUpperCase(nomefile));
 ...
 stream = fdopen(handle, "r");
  ...
  do
   {
	fscanf(stream,"%le %le", &wave, &lips);
   {
   if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500)) wave=wave*10;
   if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str())) && (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str()))) SDIAppForm->Series1->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ',clBlack);
   }
   }
   while (!feof(stream));
  fclose(stream);
  ...
  stream = fopen ( (nomefile).c_str() , "rb" );
...
float sx;
fread(&sx, sizeof(float), 1, stream);
float version=sx;
if (version < 70) {
fread(&sx, sizeof(float), 1, stream);
float start_w=sx;
...
fread(&sx, sizeof(float), 1, stream);
fread(&sx, sizeof(float), 1, stream);
for (int nq = 0; nq < npoints; nq++) {
   int qq = fread(&sx, 4, 1, stream);
  float wave=start_w+nq*res_w+quadr*nq*nq+cub*nq*nq*nq;
  float lips=sx;
 if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500)) wave=wave*10;
   if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str())) && (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str()))) SDIAppForm->Series1->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ',clBlack);
   }
   ...
   close(handle);
	...
	char* buffer = (char*) malloc (sizeof(char)*4*73);
	  fread(buffer, sizeof(float), 73, stream);
	   fread(&sx, sizeof(float), 1, stream);
	   float start_w=sx;
		   fread(&sx, sizeof(float), 1, stream);
	   float res_w=sx;
	 fread(&sx, sizeof(float), 1, stream);
	   ...
	   float startt=sx;
		   fread(&sx, sizeof(float), 1, stream);
	   float endd=sx;
	 char* buffer2 = (char*) malloc (sizeof(char)*4*19);
	   fread(buffer2, sizeof(float), 19, stream);
	   for (int nq = startt; nq < endd; nq++) {
   int qq = fread(&sx, 4, 1, stream);
  float wave=start_w+nq*res_w+quadr*nq*nq+cub*nq*nq*nq;
  float lips=sx;
 if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500)) wave=wave*10;
   ...
   for (int i=0; i < SDIAppForm->Series1->Count(); i++)
  {
   SDIAppForm->Series1->YValues->Value[i]-=ofs;
	 ...
	 double mm,mx;
SDIAppForm->Chart1->LeftAxis->CalcMinMax(mm,mx);
SDIAppForm->Chart1->LeftAxis->SetMinMax(0.0,mx);
	SDIAppForm->Chart1->Refresh();
  }
...
SDIAppForm->Chart1->Refresh();
mixx= SDIAppForm->Chart1->BottomAxis->Minimum;
maxx= SDIAppForm->Chart1->BottomAxis->Maximum;
   SDIAppForm->ScrollBar1->SetParams((maxx+mixx)/2,mixx,maxx);
   SDIAppForm->Caption = "Current File:" + ExtractFileName(nomefile);
   SDIAppForm->Edit5->Text=1;
  }

  float findfirstvalue(AnsiString nomefile)
 {
 int handle;
...
delta0=atof(AnsiString(OKBottomDlg->Edit8->Text).c_str());
...
SDIAppForm->Chart1->BottomAxis->Automatic=false;
  ...
  group=1;
  oldj=j;
  if (OKBottomDlg->CheckBox9->Checked) {
...
for (I = 1; I < Form9->StringGrid1->ColCount; I++)
	for (J = 1; J < Form9->StringGrid1->RowCount; J++)
	  ...
	  Form31->SaveDialog1->Title="Save OPC";
Form31->SaveDialog1->Filter="Data|*.OPC";
Form31->SaveDialog1->FileName = "*.OPC";
Form31->SaveDialog1->InitialDir=ExtractFilePath(Application->ExeName);
char stringa[80]="The file already exists. Overwrite?";
if (Form31->SaveDialog1->Execute())
		...
		if (FileExists(Form31->SaveDialog1->FileName))
				...
				if (MessageDlg(stringa, mtConfirmation, TMsgDlgButtons() << mbYes << mbNo <<
			mbCancel, 0) != IDYES) {
			Form31->Button3->Caption="DISABLED";
			return;   }
		}

FILE *out;
out = fopen(AnsiString(Form31->SaveDialog1->FileName).c_str(), "w");
				}
...
for (int i=0; i < Form9->Query1->RecordCount; i++)
		...
		Form9->StringGrid1->Cells[1][i+1]=sname;
			  Form9->Chart1->Series[i]->Name=sname;
...
(wave-Form10->Series1->XValues->Value[k]);
			 ...
			 if (Form31->Button3->Caption=="ENABLED") {
						 ...
						 float dlt=StrToFloat(OKBottomDlg->Edit14->Text)+StrToFloat(OKBottomDlg->Edit15->Text);
						  float yp=-Ek/tt+ass+log(csp/cs);
						  float delta=yp-log(inte/yyy/fabs(aki*gk));
							  yyy*=exp(-delta);
							  FILE *out;
					out = fopen(AnsiString(Form31->SaveDialog1->FileName).c_str(), "a");
							  fprintf(out,"%f %f \n",wave-dlt,yyy);
							  fprintf(out,"%f %f \n",wave+dlt,yyy);
							  fclose(out);
						  }
...
// Calcolo Temperatura media
     ...
     for (int i=0; i < Form9->Query1->RecordCount; i++)
        {
		ktt=atof(AnsiString(Form9->StringGrid1->Cells[3][i+1]).c_str());
		if ((ktt > atof(AnsiString(OKBottomDlg->Edit20->Text).c_str())) & (ktt < atof(AnsiString(OKBottomDlg->Edit21->Text).c_str())))
                {
				int lins=atoi(AnsiString(Form9->StringGrid1->Cells[2][i+1]).c_str());
                tmed+=lins*ktt;
                num+=lins;
                }
        Form9->Query1->Next();
     ...
     if (num!=0.0)
        ...
        {
		tmed=atof(AnsiString(Form9->Edit1->Text).c_str());
        }
     char tdm[10];
     sprintf(tdm,"%1.4f",tmed);
     Form9->Edit7->Text=tdm;

	 if ((tmed < atof(AnsiString(OKBottomDlg->Edit20->Text).c_str())) | (tmed > atof(AnsiString(OKBottomDlg->Edit21->Text).c_str())))
		{
		tmed=atof(AnsiString(Form9->Edit1->Text).c_str());
        }
        Form9->Edit13->Text=tmed;
 ...
 for (int i=0; i < Form9->Query1->RecordCount; i++)
        ...
        switch (Form9->RadioGroup1->ItemIndex)
                {
                case 0:
				kappati=atof(AnsiString(Form9->StringGrid1->Cells[3][i+1]).c_str());
                break;
                case 1:
                kappati=tmed;
                break;
                case 2:
				kappati=atof(AnsiString(Form9->Edit1->Text).c_str());
                Form9->Edit7->Text=Form9->Edit1->Text;
                break;
                case 3:
                int ix=Form9->ComboBox1->ItemIndex;
				kappati=atof(AnsiString(Form9->StringGrid1->Cells[3][ix+1]).c_str());
                Form9->Edit1->Text=kappati;
                break;
                }

		if (((kappati < atof(AnsiString(OKBottomDlg->Edit20->Text).c_str())) | (kappati > atof(AnsiString(OKBottomDlg->Edit21->Text).c_str())) ) & (Form9->RadioGroup1->ItemIndex == 0))
                {
                kappati= tmed;
                }
        ...
        for (int p=0; p < Form9->Chart1->Series[i]->Count(); p++)
                {
				erro=max(atof(AnsiString(Form9->Chart1->Series[i]->XLabel[p]).c_str()),1.e-20);
                erroq=erro*erro;
        ...
        switch(atoi(io.c_str()))
                {
                case 1:
				sprintf(sname,"Fit%sI",AnsiString(Trim(spe)).c_str());
                break;
                case 2:
                sprintf(sname,"Fit%sII",AnsiString(Trim(spe)).c_str());
                break;
                }
  ...
  */
  ...
  // calcolo database con database estratto da Payling
         int oin;
 ...
 gltstar=atof(AnsiString(Form9->Edit7->Text).c_str());
 if(Form9->CheckBox1->Checked == false)
        ...
        for (int i=0; i < Form9->Query2->RecordCount;i++)
                ...
                for (int j=0; j < Form9->Query1->RecordCount;j++)
                        ...
                        if  ((Form9->StringGrid1->Cells[1][j+1]==sname))
                                {
								t1=atof(AnsiString(Form9->StringGrid1->Cells[3][j+1]).c_str());
								pop1=atof(AnsiString(Form9->StringGrid1->Cells[4][j+1]).c_str());
								z1=atof(AnsiString(Form9->StringGrid1->Cells[5][j+1]).c_str());
                                break;
                                }
                        Form9->Query1->Next();
                        }
                ...
                for (int j=0; j < Form9->Query1->RecordCount;j++)
                        ...
                        if  ((Form9->StringGrid1->Cells[1][j+1]==sname))
                                {
								t2=atof(AnsiString(Form9->StringGrid1->Cells[3][j+1]).c_str());
								pop2=atof(AnsiString(Form9->StringGrid1->Cells[4][j+1]).c_str());
								z2=atof(AnsiString(Form9->StringGrid1->Cells[5][j+1]).c_str());
                                break;
                                }
                        Form9->Query1->Next();
                        }
                ...
                if ((lin1*lin2) !=0)
                        ...
                        switch(Form9->RadioGroup1->ItemIndex)
                                ...
                                break;
                                case 2:
								kti=atof(AnsiString(Form9->Edit1->Text).c_str());
                                break;
                                case 3:
                                kti=t1;
                                break;
        ...
        for (int i=0; i < Form9->Query2->RecordCount; i++)
                {
				dtt=atof(AnsiString(Form9->StringGrid2->Cells[4][i+1]).c_str());
                if (dtt !=0.0)
                        {
                        dmed+=dtt;
                        dnum++;
        ...
        if (dnum !=0.0)
                ...
                {
				dmed=atof(AnsiString(Form9->Edit2->Text).c_str());
                }

		if ((dmed < atof(AnsiString(OKBottomDlg->Edit18->Text).c_str())) | (dmed > atof(AnsiString(OKBottomDlg->Edit19->Text).c_str())))
                {
				dmed=atof(AnsiString(Form9->Edit2->Text).c_str());
                }
        ...
        for (int i=0; i < Form9->Query2->RecordCount; i++)
                ...
                switch (Form9->RadioGroup2->ItemIndex)
                        {
                        case 0:
						densel=atof(AnsiString(Form9->StringGrid2->Cells[4][i+1]).c_str());
                        break;
                        case 1:
                        densel=dmed;
                        break;
                        case 2:
						densel=atof(AnsiString(Form9->Edit2->Text).c_str());
                        dmed=densel;
                        break;
                        case 3:
                        int ix=Form9->ComboBox2->ItemIndex;
						densel=atof(AnsiString(Form9->StringGrid2->Cells[4][ix+1]).c_str());
                        char dm1[10];
				...
				if (((densel < atof(AnsiString(OKBottomDlg->Edit18->Text).c_str())) | (densel > atof(AnsiString(OKBottomDlg->Edit19->Text).c_str())) ) && (Form9->RadioGroup2->ItemIndex == 0))
						{
						densel=atof(AnsiString(Form9->Edit2->Text).c_str());
                        }
        ...
        if (Form20->Table1->RecordCount !=0)
                ...
                Form20->StringGrid1->Cells[5][0]="K*";
                Form20->StringGrid1->Cells[6][0]="Uncertainty";
                ...
                for (int j=0; j < Form20->Table1->RecordCount;j++)
                        ...
                        sprintf(sqls,"SELECT * FROM '%s' where specie = '%s' and ion = %i",AnsiString(Form6->Table1->TableName).c_str(),Form20->Table1->FieldByName("Element")->AsString,Form20->Table1->FieldByName("Ion")->AsInteger);
                        Form6->Query1->SQL->Clear();
                        Form6->Query1->SQL->Add(sqls);
                        Form6->Query1->Open();
                        ...
                        if (Form6->Query1->RecordCount !=0)
                                ...
                                for (int q=0; q < Form6->Query1->RecordCount;q++)
                                        ...
                                        if (wwa == wwac && Form6->Query1->FieldByName("inte")->AsFloat !=0.0)
                                                ...
                                                char acc[5];
                                                sprintf(acc,"%s",Form20->Table1->FieldByName("acc")->AsString);
												...
												double nmin=log10(atof(AnsiString(OKBottomDlg->Edit18->Text).c_str()));
												double nmax=log10(atof(AnsiString(OKBottomDlg->Edit19->Text).c_str()));
                                                float xacc=0.0001;
												char sqls[300];
                                                ...
                                                for (int b=0; b < Form9->Query3->RecordCount; b++)
                                                        ...
                                                        if (abs(Form20->Table1->FieldByName("wavelength")->AsFloat-Form9->Query3->FieldByName("asswavelen")->AsFloat) < 0.01)
                                                                {
                                                                glwlor=fabs(Form9->Query3->FieldByName("wl")->AsFloat);
                                                        ...
                                                        //        glwlor=sqrt(pow(wgau,2)+pow(glwlor,2)-pow(atof(OKBottomDlg->Edit16->Text.c_str()),2));
                                                         //        double glwlor1=sqrt(pow(wgau,2)+pow(glwlor,2)-pow(atof(OKBottomDlg->Edit16->Text.c_str()),2));
															   double winst= atof(AnsiString(OKBottomDlg->Edit16->Text).c_str())/5000.*(Form9->Query3->FieldByName("asswavelen")->AsFloat);
                                                                glwlor=wtot-pow(winst,2)/wtot;

                                                          /*  calcolo k*   */

                                                                double aki=fabs(Form9->Query3->FieldByName("aki")->AsFloat);
                                                                int gk=Form9->Query3->FieldByName("gk")->AsInteger;
                                                                sprintf(sqls,"SELECT Ei FROM %s WHERE ion=%i AND [wavelen] BETWEEN %f AND %f",Form20->Table1->FieldByName("Element")->AsString,Form20->Table1->FieldByName("Ion")->AsInteger,wwa-0.04,wwa+0.04);
                                                                ...
                                                                /* calcolo funzione di partizione a T=10000K */



                                                     /*           sprintf(sqls,"SELECT DISTINCT Ei,gi FROM livelli WHERE element='%s' AND ion=%i ORDER BY Ei",Form20->Table1->FieldByName("Element")->AsString,Form20->Table1->FieldByName("Ion")->AsInteger);
                                                                Form9->Query4->SQL->Clear();
																Form9->Query4->SQL->Add(sqls);
                                                                Form9->Query4->Open();
                                                                Form9->Query4->First();
                                                                ...
                                                                float gi=Form9->Query4->FieldValues["gi"];
                                                                float ei=Form9->Query4->FieldValues["Ei"];
                                                                zstar+=gi*exp(-ei/7200);
                                                                Form9->Query4->Next();
                                                                }
                                                                }
                                                                if (zstar == 0.0)
                                                                {
                                                                zstar=2.0+exp(-2.0*11600/10000);
                                                                }
                                                                /*

                                                                int oin;
                                                                if (Form20->Table1->FieldByName("Ion")->AsInteger==1) oin=1; else oin=2;
                                                        */

                                                                float zstar=zZ(Form20->Table1->FieldByName("Element")->AsString,Form20->Table1->FieldByName("Ion")->AsInteger,0.862);
                                                                ...
                                                                /* fine calcolo funzione di partizione*/

                                                                kstar=8.4E-14*aki*gk*pow(wwa,4)*exp(-Ei/7200)/(glwlor*zstar);
												...
												double strk=pow(10.,(double)rtbis(test, nmin, nmax, xacc));
                                                double strk1=glwlor/2./glww*1e16;
                                                char stark[20];
                                                sprintf(stark,"%1.2E",strk);
                                                Form20->StringGrid1->Cells[4][jjj]=stark;
                                                char kstar1[20];
                                                sprintf(kstar1,"%1.2E",kstar);
                                                Form20->StringGrid1->Cells[5][jjj]=kstar1;
                                                Form20->StringGrid1->Cells[6][jjj]=acc;
                                                break;
                                                }
                                        // Form20->StringGrid1->RowCount=max(jjj+3,2);
                                        Form6->Query1->Next();
                                        }
                                }
                        Form20->Table1->Next();
  ...
  void report(AnsiString reportnam, AnsiString filenam, AnsiString templatenam)
{
  ...
  {
  case 0:
  sprintf(lab,"-",AnsiString(Form6->Edit1->Text).c_str());
  break;
  case 1:
   sprintf(lab,"%sI",AnsiString(Form6->Edit1->Text).c_str());
   break;
  case 2:
   sprintf(lab,"%sII",AnsiString(Form6->Edit1->Text).c_str());
  break;
  }
...
int trt, roh;
 ...
 stream = fdopen(handle, "r");
  do
   ...
   if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str()))  & (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str())))
  {
 MySeries->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ',colo[sc-6]);
  }
 else
 {
 }
...
{
SDIAppForm->CompareClick(Sender);
}
//---------------------------------------------------------------------------
 void exc()
 {
...
(xx-SDIAppForm->Series1->XValues->Value[w]);
 break;
	  ...
	  switch(k)
                {
                case 0:
				a[2+k*4]=atof(AnsiString(Form14->Edit6->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit7->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit8->Text).c_str())/2.;
                if (Form14->CheckBox6->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
                if (Form14->CheckBox7->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
				if (Form14->CheckBox8->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
                case 1:
				a[2+k*4]=atof(AnsiString(Form14->Edit9->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit10->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit11->Text).c_str())/2.;
				if (Form14->CheckBox9->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
                if (Form14->CheckBox10->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
                if (Form14->CheckBox11->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
				case 2:
				a[2+k*4]=atof(AnsiString(Form14->Edit12->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit13->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit14->Text).c_str())/2.;
                if (Form14->CheckBox12->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
				if (Form14->CheckBox13->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
                if (Form14->CheckBox14->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
				case 3:
				a[2+k*4]=atof(AnsiString(Form14->Edit15->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit16->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit17->Text).c_str())/2.;
                if (Form14->CheckBox15->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
                if (Form14->CheckBox16->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
				if (Form14->CheckBox17->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
                case 4:
				a[2+k*4]=atof(AnsiString(Form14->Edit18->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit19->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit20->Text).c_str())/2.;
				if (Form14->CheckBox18->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
                if (Form14->CheckBox19->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
                if (Form14->CheckBox20->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
				case 5:
				a[2+k*4]=atof(AnsiString(Form14->Edit21->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit22->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit23->Text).c_str())/2.;
                if (Form14->CheckBox21->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
				if (Form14->CheckBox22->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
                if (Form14->CheckBox23->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
                case 6:
				a[2+k*4]=atof(AnsiString(Form14->Edit24->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit25->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit26->Text).c_str())/2.;
                if (Form14->CheckBox24->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
                if (Form14->CheckBox25->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
				if (Form14->CheckBox26->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
                case 7:
				a[2+k*4]=atof(AnsiString(Form14->Edit27->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit28->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit29->Text).c_str())/2.;
				if (Form14->CheckBox27->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
                if (Form14->CheckBox28->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
                if (Form14->CheckBox29->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[2+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
				case 8:
				a[2+k*4]=atof(AnsiString(Form14->Edit30->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit31->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit32->Text).c_str())/2.;
                if (Form14->CheckBox30->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
				if (Form14->CheckBox31->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
                if (Form14->CheckBox32->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
                case 9:
				a[2+k*4]=atof(AnsiString(Form14->Edit33->Text).c_str()); a[3+k*4]=atof(AnsiString(Form14->Edit34->Text).c_str())/2.; a[4+k*4]=atof(AnsiString(Form14->Edit35->Text).c_str())/2.;
				if (Form14->CheckBox33->Checked==true) {ia[2+k*4]=0; origa[2+k*4]=-1*a[2+k*4];}else{origa[2+k*4]=a[2+k*4];}
				if (Form14->CheckBox34->Checked==true) {ia[3+k*4]=0; origa[3+k*4]=-1*a[3+k*4];}else{origa[3+k*4]=a[3+k*4];}
				if (Form14->CheckBox35->Checked==true) {ia[4+k*4]=0; origa[4+k*4]=-1*a[4+k*4];}else{origa[4+k*4]=a[4+k*4];}break;
                }
	  ...
	  /* intensità iniziale */
          ...
          /* blocchi generici wav, wg, wl*/

		  if (OKBottomDlg->CheckBox4->Checked)ia[2+k*4]=0;
          if (OKBottomDlg->CheckBox5->Checked)ia[3+k*4]=0;
          if (OKBottomDlg->CheckBox6->Checked)ia[4+k*4]=0;

   /* riempio tabella condizioni iniziali*/
 /*   switch(k)
                {
				case 0:
                Form14->Edit6->Text=a[2+k*4]; Form14->Edit7->Text=a[3+k*4]; Form14->Edit8->Text=a[4+k*4]; break;
				case 1:
                Form14->Edit9->Text=a[2+k*4]; Form14->Edit10->Text=a[3+k*4]; Form14->Edit11->Text=a[4+k*4];break;
				case 2:
                Form14->Edit12->Text=a[2+k*4]; Form14->Edit13->Text=a[3+k*4]; Form14->Edit14->Text=a[4+k*4];break;
				case 3:
                Form14->Edit15->Text=a[2+k*4]; Form14->Edit16->Text=a[3+k*4]; Form14->Edit17->Text=a[4+k*4];break;
				case 4:
                Form14->Edit18->Text=a[2+k*4]; Form14->Edit19->Text=a[3+k*4]; Form14->Edit20->Text=a[4+k*4];break;
				case 5:
				Form14->Edit21->Text=a[2+k*4]; Form14->Edit22->Text=a[3+k*4]; Form14->Edit23->Text=a[4+k*4];break;
				case 6:
				Form14->Edit24->Text=a[2+k*4]; Form14->Edit25->Text=a[3+k*4]; Form14->Edit26->Text=a[4+k*4];break;
				case 7:
				Form14->Edit27->Text=a[2+k*4]; Form14->Edit28->Text=a[3+k*4]; Form14->Edit29->Text=a[4+k*4];break;
				case 8:
				Form14->Edit30->Text=a[2+k*4]; Form14->Edit31->Text=a[3+k*4]; Form14->Edit31->Text=a[4+k*4];break;
				case 9:
				Form14->Edit33->Text=a[2+k*4]; Form14->Edit34->Text=a[3+k*4]; Form14->Edit35->Text=a[4+k*4];break;
				}
 ...
 */
		...
		alpha=matrix(0,na,0,na);
         covar=matrix(0,na,0,na);
               ...
               chisq=1.0E18;
			   int maxcounts;
				maxcounts=atof(AnsiString(OKBottomDlg->Edit22->Text).c_str());
                int count;
            int maxcount;
		   maxcount=atof(AnsiString(OKBottomDlg->Edit22->Text).c_str());
			   oldchisq=2.0E18;
               chisq=1.0E18;
 ...
 // Form14->ModalResult=0;
	Form14->Close();
	 Form14->ShowModal();
        int re;
	   re = atoi(AnsiString(SDIAppForm->Edit7->Text).c_str());
 ...
 // Inizio reset
  ...
  if (atof(AnsiString(SDIAppForm->Series4->XLabel[SDIAppForm->Series4->FirstValueIndex+k]).c_str()) !=0.0)
 ...
 // fine reset
  ...
  //fine if
		   ...
		   ran=atof(AnsiString(OKBottomDlg->Edit9->Text).c_str());
 ...
 Form14->StringGrid1->Cells[1][1]=kpt;
  ...
  Form14->StringGrid1->Cells[1][2]=kpt;
   ...
   int  fi=SDIAppForm->Series1->FirstValueIndex;
    int  la=SDIAppForm->Series1->LastValueIndex;
 for (int qq=0 ,  w= fi; w< la-1; w++ , qq++) Form18->Series1->AddXY(SDIAppForm->Series1->XValues->Value[w],SDIAppForm->Series1->YValues->Value[w]-SDIAppForm->Series3->YValues->Value[qq]);
 ...
 //      float dx=(xx[upoints-1]-xx[0])/upoints;
  ...
  float *w=vector(0,3);
  float dw0=atof(AnsiString(OKBottomDlg->Edit8->Text).c_str());
      for (g=0;g<group;g++)
        {
		 w[0]= a[1+4*g];
         w[1]= a[2+4*g];
  ...
  // result=a[4+4*g]*a[1+4*g]*sqrt(3.14151)*sqrt(log(2.))/a[3+4*g];
		...
		double inst;
	   if (OKBottomDlg->CheckBox9->Checked) {
		inst=atof(AnsiString(OKBottomDlg->Edit16->Text).c_str())/5000.*a[2+4*g];
		  }
	   else inst= atof(AnsiString(OKBottomDlg->Edit16->Text).c_str());
  ...
  // fine counts
    ...
    Form14->Close();
     Form14->ShowModal();
	  ...
	  int re1;
	  re1 = atoi(AnsiString(SDIAppForm->Edit7->Text).c_str());
 ...
 // Inizio scrittura template
  ...
  Form6->Table1->RecNo=gg+1;
       Form6->Table1->Edit();
/*     Form6->Table1->FieldByName("wavelen")=origa[2+4*g];      */
           Form6->Table1->FieldValues["inte"]=Form14->Series1->YValues->Value[g];
      if (origa[2+4*g]<0) {Form6->Table1->FieldValues["fitwavelen"]=-1*a[2+4*g];}
	  else {Form6->Table1->FieldValues["fitwavelen"]=a[2+4*g];}
      if (origa[3+4*g]<0) {Form6->Table1->FieldValues["wg"]=-2*a[3+4*g];}
      else {Form6->Table1->FieldValues["wg"]=2*a[3+4*g];}
      if (origa[4+4*g]<0) {Form6->Table1->FieldValues["wl"]=-2*a[4+4*g];}
      else {Form6->Table1->FieldValues["wl"]=2*a[4+4*g];}
...
Form14->Button7->Enabled=true;
Form14->Button9->Visible=true;
Form14->Button9->Enabled=true;

				 TObject *sen;
			 Form14->Button1Click(sen);
...
int j,k,upoints,*ia;
 ...
 int group = (SDIAppForm->Series4->LastValueIndex - SDIAppForm->Series4->FirstValueIndex) +1;
   ...
   {
   Form33->Show();
   Form33->Memo1->Lines->Append("Too many lines for fitting (> 10)");
   char txt[200];
   float axmn,axmx;
   axmn=SDIAppForm->Chart1->BottomAxis->Minimum;
   axmx=SDIAppForm->Chart1->BottomAxis->Maximum;
   sprintf(txt,"   [ %i lines between %.2f and %.2f A]",group, axmn, axmx);
   Form33->Memo1->Lines->Append(txt);
   Form33->Refresh();
	 return;
   }
...
(xx-SDIAppForm->Series1->XValues->Value[w]);
 ...
 //  Form14->Show();
	Form33->Show();
   Form33->Memo1->Lines->Append("Spectral range too wide ");
   char txt[200];
   float axmn,axmx;
   axmn=SDIAppForm->Chart1->BottomAxis->Minimum;
   axmx=SDIAppForm->Chart1->BottomAxis->Maximum;
   sprintf(txt,"   [ %i line(s) between %.2f and %.2f A]",group, axmn, axmx);
   Form33->Memo1->Lines->Append(txt);
   Form33->Refresh();
	 return;
   }
		...
		alpha=matrix(0,na,0,na);
         covar=matrix(0,na,0,na);
			   ...
			   int maxcounts;
                ...
                int count;
            int maxcount;
		   maxcount=atof(AnsiString(OKBottomDlg->Edit22->Text).c_str())*5;
			   oldchisq=2.0E18;
			   chisq=1.0E18;
 ...
 // Inizio if
			...
			{
			 Form33->Show();
			 Form33->Memo1->Lines->Append("Parameters unstable. Fit aborted.");
			    char txt[200];
				float axmn,axmx;
			  axmn=SDIAppForm->Chart1->BottomAxis->Minimum;
			   axmx=SDIAppForm->Chart1->BottomAxis->Maximum;
 ...
 //      float dx=(xx[upoints-1]-xx[0])/upoints;
  ...
  // fine counts
    ...
    //    Form14->FormStyle=fsStayOnTop;
          Form33->Show();
			 Form33->Memo1->Lines->Append("More fitting possible.");
			    char txt[200];
			   float axmn,axmx;
			   axmn=SDIAppForm->Chart1->BottomAxis->Minimum;
			   axmx=SDIAppForm->Chart1->BottomAxis->Maximum;
...
*y=a[0]+a[m]*x;
 dyda[0]=1.0;
  dyda[m]=x;
   ...
   int npeaks=(m-1)/4.;
	 float gmin= atof(AnsiString(OKBottomDlg->Edit16->Text).c_str());
for (i=0;i<npeaks;i++)
{
float ampl=a[1 + 4*i];
float pos=a[2 + 4*i];
...
void plotvoigt0(float x, float a[], float* y, int na)
{
  ...
  *y=a[0]+a[na]*x;
   ...
   int group=(na-1)/4.0;
	...
	int sc=atoi(AnsiString(SDIAppForm->Edit13->Text).c_str());
...
void __fastcall TSDIAppForm::CompareClick(TObject *Sender)
{
void compfile(AnsiString nomefile);
  ...
  OpenDialog->FileName = "";
	...
	if (OpenDialog->Execute())
  {
	for (int I = 0; I < SDIAppForm->OpenDialog->Files->Count; I ++)
	{
  SaveDialog->FileName = AnsiString(OpenDialog->Files->Strings[I].c_str());

   for (int y=1;y<Form13->StringGrid1->RowCount;y++)
	 {
	 if (Form13->StringGrid1->Cells[1][y]=="")
	{
	Form13->StringGrid1->Cells[1][y]=AnsiString(OpenDialog->Files->Strings[I].c_str());
	Form13->StringGrid1->Refresh();
	break;
	}
	  }

  compfile(AnsiString(OpenDialog->Files->Strings[I].c_str()));
		fp=(ExtractFilePath(AnsiString(OpenDialog->Files->Strings[I].c_str())));
...
void __fastcall TSDIAppForm::Active_SpectraClick(TObject *Sender)
{
Form13->StringGrid1->ColWidths[0]=12;
Form13->StringGrid1->ColWidths[1]=400;
Form13->Edit1->Text="";
...
{ /*    bool D3;
   D3=SDIAppForm->Chart1->View3D;
   if (D3==false)  {
   SDIAppForm->Chart1->View3D=true;
...
int colore;
   ...
   SDIAppForm->ColorDialog1->Color=SDIAppForm->Chart1->BackColor;
    if (SDIAppForm->ColorDialog1->Execute())
    colore = SDIAppForm->ColorDialog1->Color;
	SDIAppForm->Chart1->Color=colore;
    }
    return;        
}
//---------------------------------------------------------------------------
...
void __fastcall TSDIAppForm::ClearGraphClick(TObject *Sender)
{
char stringa[80]="This will clear the spectra and close the current template. Continue?";
...
void __fastcall TSDIAppForm::Statistics1Click(TObject *Sender)
{
Form28->Show();
       double xm=0;
         double ym=0;
          ...
          double xmax=-1e16;
          int npoints;
       for (int w=Series1->FirstValueIndex;w<Series1->LastValueIndex+1;w++)
	  {
      if (SDIAppForm->Series1->YValues->Value[w] < ymin) ymin=SDIAppForm->Series1->YValues->Value[w];
       if (SDIAppForm->Series1->YValues->Value[w] > ymax) ymax=SDIAppForm->Series1->YValues->Value[w];
    if (SDIAppForm->Series1->XValues->Value[w] < xmin) xmin=SDIAppForm->Series1->XValues->Value[w];
	if (SDIAppForm->Series1->XValues->Value[w] > xmax) xmax=SDIAppForm->Series1->XValues->Value[w];
      }
      npoints=Series1->LastValueIndex-Series1->FirstValueIndex+1;
      Form28->Label17->Caption=FormatFloat("#0.0000",xmin);
      Form28->Label18->Caption=FormatFloat("#0.0000",xmax);
      Form28->Label19->Caption=FormatFloat("#0.0000",ymin);
      Form28->Label20->Caption=FormatFloat("#0.0000",ymax);
      Form28->Label22->Caption=FormatFloat("#0",npoints);

       double norm=0;
       for (int w=Series1->FirstValueIndex;w<Series1->LastValueIndex+1;w++)
      {
      norm+=(SDIAppForm->Series1->YValues->Value[w]-ymin);
      }

 for (int w=Series1->FirstValueIndex;w<Series1->LastValueIndex+1;w++)
      {
	  xm+= SDIAppForm->Series1->XValues->Value[w]*(SDIAppForm->Series1->YValues->Value[w]-ymin);
	  ym+= SDIAppForm->Series1->YValues->Value[w];
	}

	  xm/=norm;
      ym/=npoints;
      Form28->Label4->Caption=FormatFloat("#0.0000",xm);
      Form28->Label9->Caption=FormatFloat("#0.0000",ym);

      double xqm=0;
      double yqm=0;
       for (int w=Series1->FirstValueIndex;w<Series1->LastValueIndex+1;w++)
      {
      xqm+= pow(SDIAppForm->Series1->XValues->Value[w]-xm,2)*(SDIAppForm->Series1->YValues->Value[w]-ymin);
      yqm+= pow(SDIAppForm->Series1->YValues->Value[w]-ym,2);
  }
      xqm/=norm;
      yqm/=npoints;
      Form28->Label5->Caption=FormatFloat("#0.0000",xqm);
      Form28->Label10->Caption=FormatFloat("#0.0000",yqm);
            double sqw=0;
            double sqwy=0;
       for (int w=Series1->FirstValueIndex;w<Series1->LastValueIndex+1;w++)
      {
      sqw+= pow(SDIAppForm->Series1->XValues->Value[w]-xm,3)*(SDIAppForm->Series1->YValues->Value[w]-ymin);
      sqwy+= pow(SDIAppForm->Series1->YValues->Value[w]-ym,3);
    }
      sqw/=norm;
      sqwy/=npoints;
      sqw/=pow(xqm,1.5);
      sqwy/=pow(yqm,1.5);
      Form28->Label6->Caption=FormatFloat("#0.0000",sqw);
          Form28->Label11->Caption=FormatFloat("#0.0000",sqwy);
             double kur=0;
             double kury=0;
       for (int w=Series1->FirstValueIndex;w<Series1->LastValueIndex+1;w++)
      {
      kur+= pow(SDIAppForm->Series1->XValues->Value[w]-xm,4)*(SDIAppForm->Series1->YValues->Value[w]-ymin);
       kury+= pow(SDIAppForm->Series1->YValues->Value[w]-ym,4);
     }
      kur/=norm;
      kury/=npoints;
      kur/=pow(xqm,2);
      kury/=pow(yqm,2);
      Form28->Label7->Caption=FormatFloat("#0.0000",kur-3);
      Form28->Label12->Caption=FormatFloat("#0.0000",kury-3);
}
//---------------------------------------------------------------------------


void __fastcall TSDIAppForm::Apply1Click(TObject *Sender)
{
/*
Form29->Series1->Clear();
for (int i=0; i < SDIAppForm->Series2->Count()-1; i++)
{
 for (int j=0; j < SDIAppForm->Series1->Count(); j++)
 {
 if (SDIAppForm->Series1->XValues->Value[j] >= SDIAppForm->Series2->XValues->Value[i] && SDIAppForm->Series1->XValues->Value[j]<= SDIAppForm->Series2->XValues->Value[i+1])
 {
 double fatt= (SDIAppForm->Series2->YValues->Value[i+1]-SDIAppForm->Series2->YValues->Value[i])/(SDIAppForm->Series2->XValues->Value[i+1]-SDIAppForm->Series2->XValues->Value[i]);
 double yyy=SDIAppForm->Series1->YValues->Value[j]-SDIAppForm->Series2->YValues->Value[i]-(SDIAppForm->Series1->XValues->Value[j]-SDIAppForm->Series2->XValues->Value[i])*fatt;
  if (yyy < 0) yyy=0;
 Form29->Series1->AddXY(SDIAppForm->Series1->XValues->Value[j],yyy,' ',clBlack);
 }
 }
}
Form29->Series2->Clear();
for (int u=0;u<SDIAppForm->Series1->Count();u++) Form29->Series2->AddXY(SDIAppForm->Series1->XValues->Value[u],SDIAppForm->Series1->YValues->Value[u],' ',clBlack);
SDIAppForm->Series1->Clear();
for (int u=0;u<Form29->Series1->Count();u++) SDIAppForm->Series1->AddXY(Form29->Series1->XValues->Value[u],Form29->Series1->YValues->Value[u],' ',clBlack);
swap(Form29->Series1,Form29->Series2);
SDIAppForm->Series1->Active=true;
 Chart1->BottomAxis->Automatic=true;
  Chart1->LeftAxis->Automatic=true;
  */
...
void __fastcall TSDIAppForm::Remove1Click(TObject *Sender)
{
/*
Form29->Series2->Clear();
for (int u=0;u<SDIAppForm->Series1->Count();u++) Form29->Series2->AddXY(SDIAppForm->Series1->XValues->Value[u],SDIAppForm->Series1->YValues->Value[u],' ',clBlack);
...
void __fastcall TSDIAppForm::Clear1Click(TObject *Sender)
{
SDIAppForm->Series2->Clear();
SDIAppForm->Series2->Active=false;
}
...
void __fastcall TSDIAppForm::N10Click(TObject *Sender)
{
TVarRec vrs[1];
SaveDialog->FileName = "";
SaveDialog->Filter = "Baseline files (*.bsl)|*.bsl|All Files (*.*)|*.*";
          char stringa[80]="The file already exists. Overwrite?";
	if (SaveDialog->Execute())
	{
  ...
  FILE *out;
   ...
   out = fopen(AnsiString(SaveDialog->FileName).c_str(), "w");
        int w;
      for (w=Series2->FirstValueIndex;w<Series2->LastValueIndex+1;w++)
      {
        fprintf(out,"%f %f\n", Series2->XValues->Value[w], Series2->YValues->Value[w]);
      }
         fclose(out);
	}
}
...
void __fastcall TSDIAppForm::Load1Click(TObject *Sender)
{
int handle;
 ...
 SDIAppForm->Chart1->BottomAxis->Automatic=true;
  SDIAppForm->Chart1->LeftAxis->Automatic=true;
     OpenDialog->FileName = "";
     OpenDialog->Filter = "Baseline files (*.bsl)|*.bsl|All Files (*.*)|*.*";
 ...
 if (OpenDialog->Execute())
  ...
  do
   {
   fscanf(stream,"%le %le", &wave, &lips);
  SDIAppForm->Series2->AddXY(wave,lips,' ',clRed);
  }
   while (!feof(stream));
  fclose(stream);
   close(handle);
...
float de=atof(AnsiString(OKBottomDlg->Edit2->Text).c_str());
  float ha=6563.;
   int group = atof(AnsiString(OKBottomDlg->Edit1->Text).c_str());

 if (group > 2 | group < 1)
 {
 ShowMessage("Number of lines must be 1 or 2");
 return;
 }
...
*/
 ...
 char stringa[80]="Calculate the electron density from H alpha line?";
...
/* inserimento condizioni iniziali*/
     ...
     {
     a[2+k*4]=6563.;
     a[1+k*4]=(SDIAppForm->Series1->MaxYValue()-a[0]-a[2+k*4]*a[na])*(a[3+k*4]+a[4+k*4])*6.28;
     }
      else
      {
      a[2+k*4]=6595.;
      a[1+k*4]=(SDIAppForm->Series1->MaxYValue()/10.-a[0]-a[2+k*4]*a[na])*(a[3+k*4]+a[4+k*4])*6.28;
      }
  ...
  MySeries->LinePen->Style=psDot;
   ...
   for ( count=0; count < maxcount; count++)
   {
       ...
       plotvoigt(xx[g],a,&y,na);
        ...
        double glwlor=2*a[4];
        double wgau=2*a[3];
        Form16->Edit2->Text=FormatFloat("#0.000",wgau);
        Form16->Edit3->Text=FormatFloat("#0.00000",glwlor);
        double wtot=sqrt(pow(glwlor/2,2)+pow(wgau,2))+glwlor/2;
		double inst;
	   if (OKBottomDlg->CheckBox9->Checked) {
		 inst=atof(AnsiString(OKBottomDlg->Edit16->Text).c_str())/5000.*a[2];
		  }
		   else inst= atof(AnsiString(OKBottomDlg->Edit16->Text).c_str());
		glwlor=wtot-pow(inst,2)/wtot;
        float ne=2.84e15*pow(glwlor,1.5);
        Form16->Edit1->Text=FormatFloat("#0.00000",ne/1.e17);
...
void __fastcall TSDIAppForm::Load_TemplateClick(TObject *Sender)
{
void loadtempl(AnsiString nome);
  SDIAppForm->OpenDialog1->Title="Load Template";
 SDIAppForm->OpenDialog1->Filter = "Templates|*.DB";
...
void __fastcall TSDIAppForm::Show_TemplateClick(TObject *Sender)
{
Form6->Show();        
}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::Find_PeaksClick(TObject *Sender)
{
void savgol(float c[], int np, int nl, int nr, int ld, int m);
float *vector(long nl,long nh);
float *a;
  ...
  float x[10000];
    ...
    Series3->AddXY(x[z],histo[z],' ',clBlack);
		...
		for (u=1;u<100;u++)
		  ...
		  float delta1=pow(a[1]-astart,2)/pow(astart,2);
		  float delta2=pow(a[2]-bstart,2)/pow(bstart,2);
		  astart=a[1];
		  bstart=a[2];
		  if (delta1<0.00001 && delta2<0.00001) {
				  break;
		  }
		  }
	...
	char wavel[80];
     float shift;
	  float threshold=atof(AnsiString(OKBottomDlg->Edit7->Text).c_str());
    ...
    {
	 if (fabs(Series2->YValues->Value[g]-a[1]) > a[2]*threshold)
     {
...
void __fastcall TSDIAppForm::TraceClick(TObject *Sender)
{
Form2->Show();        
}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::Ne_from_HClick(TObject *Sender)
{
 ...
 return;
  ...
  float de=atof(AnsiString(OKBottomDlg->Edit2->Text).c_str());
  float ha=atof(AnsiString(OKBottomDlg->Edit5->Text).c_str());
...
if (MessageDlg(stringa, mtConfirmation, TMsgDlgButtons() << mbYes << mbNo << mbCancel, 0) != IDYES)
{
  SDIAppForm->Chart1->BottomAxis->Automatic=true;
  SDIAppForm->Chart1->LeftAxis->Automatic=true;
  SDIAppForm->Chart1->Refresh();
...
/* inserimento condizioni iniziali*/
	 ...
	 if (k==0)
	 {
	 a[2+k*4]=atof(AnsiString(OKBottomDlg->Edit5->Text).c_str());
	 a[4+k*4]=atof(AnsiString(OKBottomDlg->Edit12->Text).c_str());
	 a[1+k*4]=(SDIAppForm->Series1->MaxYValue()-a[0]-a[2+k*4]*a[na])*(a[3+k*4]+a[4+k*4])*6.28;
	 }
	  else
	  {
	  a[4+k*4]=atof(AnsiString(OKBottomDlg->Edit24->Text).c_str());
	  a[2+k*4]=atof(AnsiString(OKBottomDlg->Edit6->Text).c_str());
      a[1+k*4]=(SDIAppForm->Series1->MaxYValue()/10.-a[0]-a[2+k*4]*a[na])*(a[3+k*4]+a[4+k*4])*6.28;
	  }
  ...
  MySeries->LinePen->Style=psDot;
   ...
   for ( count=0; count < maxcount; count++)
   {
	   ...
	   plotvoigt(xx[g],a,&y,na);
		...
		double glwlor=2.*a[4];
		double wgau=2.*a[3];
		Form16->Edit2->Text=FormatFloat("#0.00",wgau);
		Form16->Edit3->Text=FormatFloat("#0.00",glwlor);
		Form27->Edit2->Text=FormatFloat("#0.00",glwlor);
		double wtot=sqrt(pow(glwlor/2,2)+pow(wgau,2))+glwlor/2;
		double inst;
		if (OKBottomDlg->CheckBox9->Checked) {
		  inst=atof(AnsiString(OKBottomDlg->Edit16->Text).c_str())/5000.*a[2];
		...
		glwlor=wtot-pow(inst,2)/wtot;
		float ne=2.84e15*pow(glwlor,1.5);
		float T=11600*atof(AnsiString(OKBottomDlg->Edit25->Text).c_str());
		Form16->Edit1->Text=FormatFloat("0.00",Ne(1E17,T,glwlor)/1.e17);



   }
//---------------------------------------------------------------------------
void __fastcall TSDIAppForm::AutofitClick(TObject *Sender)
{
void autofit(float jmin, float jmax, int j);
void manfit2();
 void condizioni();
  ...
  Form33->Memo1->Lines->Append(date);
   Form33->Memo1->Lines->Append(AnsiString(Form13->StringGrid1->Cells[1][0]));
   Form33->Memo1->Lines->Append(AnsiString(Form6->Table1->TableName));
...
// if(fabs(atof(AnsiString(Form14->Edit6->Text).c_str()))!=fabs(Form6->Table1->FieldByName("fitwavelen")->AsFloat))
{
 condizioni();
   SDIAppForm->Edit7->Text="0";
manfit2();
...
j=SDIAppForm->Series4->LastValueIndex+1;
 ...
 Form33->Memo1->Lines->Append("End of Fit");
}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::Manual_FitClick(TObject *Sender)
{
void condizioni();
Form14->Width=210;
Form14->Show();
...
void __fastcall TSDIAppForm::Saha_BoltzmannClick(TObject *Sender)
{
...
{
if (Form27->StringGrid2->Cells[1][i]!="") Form27->StringGrid2->Cells[0][i]="";
}
   Form27->Refresh();
}
//---------------------------------------------------------------------------
...
void __fastcall TSDIAppForm::CF_LIBSClick(TObject *Sender)
{
void showboltz();
showboltz();
}
...
void __fastcall TSDIAppForm::Options1Click(TObject *Sender)
{
OKBottomDlg->Show();
}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::CloseTemplClick(TObject *Sender)
{
...
void __fastcall TSDIAppForm::Chart1AfterDraw(TObject *Sender)
{
dl= SDIAppForm->Chart1->BottomAxis->Minimum-SDIAppForm->Chart1->BottomAxis->Maximum;
SDIAppForm->ScrollBar1->Position=(SDIAppForm->Chart1->BottomAxis->Minimum+SDIAppForm->Chart1->BottomAxis->Maximum)/2;
}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::ScrollBar1Scroll(TObject *Sender,
      TScrollCode ScrollCode, int &ScrollPos)
{
float l=ScrollPos;
SDIAppForm->Chart1->BottomAxis->SetMinMax(l-dl/2,l+dl/2);
}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::Chart1Zoom(TObject *Sender)
{
// dl=(SDIAppForm->Chart1->BottomAxis->Maximum-SDIAppForm->Chart1->BottomAxis->Minimum);
}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::Chart1UndoZoom(TObject *Sender)
{
// dl=(SDIAppForm->Chart1->BottomAxis->Maximum-SDIAppForm->Chart1->BottomAxis->Minimum);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
   ...
   int colo[34]={clRed,clGreen,clBlue,clPurple,clGray,clMaroon,clNavy,clOlive,clLtGray,clFuchsia,clWhite,clTeal,clSilver,clLime,clAqua,clBlack+200,clRed+200,clGray+200,clAqua+200,clBlue+200,clGreen+200,clBlack+200,clMaroon+200,clNavy+200,clDkGray+200,clOlive+200,clPurple+200,clLtGray+200,clFuchsia+200,clWhite+200,clTeal+200,clSilver+200,clLime+200};
   int sc;
   sc=SDIAppForm->Chart1->SeriesCount()-5;

   if (sc==1)
        {
        MessageDlg("Only one spectrum is opened in the graph", mtWarning, TMsgDlgButtons() << mbOK, 0);
        return;
        }

   SDIAppForm->Caption = "Current File:" + ExtractFileName(Form13->StringGrid1->Cells[1][1]);
   AnsiString tp;
   tp=Form13->StringGrid1->Cells[1][0];
   for (int u=0; u < sc;u++)
   {
   Form13->StringGrid1->Cells[1][u]=Form13->StringGrid1->Cells[1][u+1];
   }
   Form13->StringGrid1->Cells[1][sc-1]=tp;
   Form13->StringGrid1->Refresh();
  ...
  for (int q=0; q < SDIAppForm->Series1->Count();q++)
   ...
   SDIAppForm->Chart1->Series[0]->Clear();
    ...
    for (int u=6; u < sc+5;u++)
    {
    SDIAppForm->Chart1->Series[u]->Active=false;
    }
     for (int q=0; q < SDIAppForm->Chart1->Series[6]->Count();q++)
                {
                SDIAppForm->Chart1->Series[0]->AddXY(SDIAppForm->Chart1->Series[6]->XValues->Value[q],SDIAppForm->Chart1->Series[6]->YValues->Value[q],' ',clBlack);
                }


    for (int u=6; u < sc+4;u++)
    {
     SDIAppForm->Chart1->Series[u]->Clear();
     for (int q=0; q < SDIAppForm->Chart1->Series[u+1]->Count();q++)
                {
                SDIAppForm->Chart1->Series[u]->AddXY(SDIAppForm->Chart1->Series[u+1]->XValues->Value[q],SDIAppForm->Chart1->Series[u+1]->YValues->Value[q],' ',colo[u-6]);
                }

    }
    SDIAppForm->Chart1->Series[sc+4]->Clear();
    for (int q=0; q < SDIAppForm->Series2->Count();q++)
                {
                SDIAppForm->Chart1->Series[sc+4]->AddXY(SDIAppForm->Series2->XValues->Value[q],SDIAppForm->Series2->YValues->Value[q],' ',colo[sc-2]);
                }
 ...
 for (int u=6; u < sc+5;u++)
    {
    SDIAppForm->Chart1->Series[u]->Active=true;
    }
    SDIAppForm->Series2->Active=false;
SDIAppForm->Chart1->Refresh();
return;
...
//---------------------------------------------------------------------------
void __fastcall TSDIAppForm::SwapButton1Click(TObject *Sender)
{
void swap();
swap();
}
...
void deleteseries()
{
 int colo[34]={clRed,clGreen,clBlue,clPurple,clGray,clMaroon,clNavy,clOlive,clLtGray,clFuchsia,clWhite,clTeal,clSilver,clLime,clAqua,clBlack+200,clRed+200,clGray+200,clAqua+200,clBlue+200,clGreen+200,clBlack+200,clMaroon+200,clNavy+200,clDkGray+200,clOlive+200,clPurple+200,clLtGray+200,clFuchsia+200,clWhite+200,clTeal+200,clSilver+200,clLime+200};
    ...
    if (rw==0)
    {
		MessageDlg("It is not possible to delete the main spectrum", mtWarning, TMsgDlgButtons() << mbOK, 0);
        return;
    }

     for (int u=rw; u < sc-1;u++)
   {
   Form13->StringGrid1->Cells[1][u]=Form13->StringGrid1->Cells[1][u+1];
   }
   Form13->StringGrid1->Cells[1][sc-1]="";
   Form13->StringGrid1->Refresh();

    SDIAppForm->Chart1->Series[rw+5]->Free();

    for (int u=rw; u < sc-1;u++)
    {

    AnsiString sd = "";
    sd.cat_sprintf("s%i", u);
    SDIAppForm->Chart1->Series[u+5]->Active=false;
	SDIAppForm->Chart1->Series[u+5]->Name=sd;
    SDIAppForm->Chart1->Series[u+5]->SeriesColor=colo[u-1];

    SDIAppForm->Series2->Clear();

    for (int q=0; q < SDIAppForm->Chart1->Series[u+5]->Count();q++)
        {
        SDIAppForm->Series2->AddXY(SDIAppForm->Chart1->Series[u+5]->XValues->Value[q],SDIAppForm->Chart1->Series[u+5]->YValues->Value[q],' ',clWhite);
        }
        SDIAppForm->Chart1->Series[u+5]->Clear();



    for (int q=0; q < SDIAppForm->Series2->Count();q++)
{
   SDIAppForm->Chart1->Series[u+5]->AddXY(SDIAppForm->Series2->XValues->Value[q],SDIAppForm->Series2->YValues->Value[q],' ',colo[u-1]);
 }
 SDIAppForm->Chart1->Series[u+5]->Active=true;
   }
   SDIAppForm->Series2->Clear();
   SDIAppForm->Chart1->Refresh();
   return;
...
//---------------------------------------------------------------------------
double Ne(float sol,float T,float d)
{
   float a=sqrt(4033.8-24.45*pow(log(T),2));
   float b=1.028E9+174576.3*T;
   float Nestart;
   do{
   Nestart=sol;
   float alfa=1/(a+b/sqrt(Nestart));
   sol=8.02E12*pow((double)d/alfa,1.5);
   } while (fabs(sol-Nestart)/sol>0.01);
   return sol;
}

void __fastcall TSDIAppForm::AutoAssign1Click(TObject *Sender)
{
     Form7->Edit2->Text=SDIAppForm->Chart1->BottomAxis->Minimum;
    Form7->Edit3->Text=SDIAppForm->Chart1->BottomAxis->Maximum;
  Form7->Show();      
}
//---------------------------------------------------------------------------
...
void __fastcall TSDIAppForm::est1Click(TObject *Sender)
{
	Form30->Show();
}
//---------------------------------------------------------------------------
...
void __fastcall TSDIAppForm::Append1Click(TObject *Sender)
{

		SDIAppForm->ToolButton2Click(Sender);
}
//---------------------------------------------------------------------------
   void appfile2(AnsiString nomefile)
 {
  ...
  int handle;
   int colo[34]={clRed,clGreen,clBlue,clPurple,clGray,clMaroon,clNavy,clOlive,clLtGray,clFuchsia,clWhite,clTeal,clSilver,clLime,clAqua,clBlack+200,clRed+200,clGray+200,clAqua+200,clBlue+200,clGreen+200,clBlack+200,clMaroon+200,clNavy+200,clDkGray+200,clOlive+200,clPurple+200,clLtGray+200,clFuchsia+200,clWhite+200,clTeal+200,clSilver+200,clLime+200};
	...
	int sc=SDIAppForm->Chart1->SeriesCount();
	  ...
	  int ru, jfin;
	 float xmx=SDIAppForm->Series1->MaxXValue();
...
int trt, roh;
  ...
  do
   ...
   {
   if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500)) wave=wave*10;
   if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str())) && (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str()))) SDIAppForm->Chart1->Series[ser]->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ', colo[cc]);
   }
   }
   while (!feof(stream));
  fclose(stream);
	  ...
	  do
   {
       fscanf(stream,"%le;%le\n", &wave,&lips);
   if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500)) wave=wave*10;
   float xf= SDIAppForm->Series2->XValues->MaxValue;
	 float xmx=SDIAppForm->Chart1->Series[ser]->XValues->MaxValue;
	  if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str())) && (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str())) && (wave > xf) && (wave > xmx)) SDIAppForm->Chart1->Series[ser]->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ', colo[cc]);
		 }
   while (!feof(stream));
  fclose(stream);
   close(handle);
 ...
 do
  {
   ...
   {
   if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500)) wave=wave*10;
   if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str())) && (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str())) && (wave > xmx)) SDIAppForm->Series1->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ',clBlack);
   }
   }
   while (!feof(stream));
  fclose(stream);
...
//   handle = open((nomefile).c_str(),O_RDONLY);
	  ...
	  do
   {
       fscanf(stream,"%le;%le\n", &wave,&lips);
   if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500)) wave=wave*10;
   float xf= SDIAppForm->Series1->XValues->MaxValue;
	  if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str())) && (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str())) && (wave > xf) && (wave > xmx))  SDIAppForm->Series1->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ',clBlack);
		 }
   while (!feof(stream));
  fclose(stream);
   close(handle);
...
maxx= SDIAppForm->Chart1->BottomAxis->Maximum;
 ...
 void appfileb(AnsiString nomefile)
 {
int handle;
 ...
 SDIAppForm->Chart1->BottomAxis->Automatic=true;
  ...
  stream = fdopen(handle, "r");
   SDIAppForm->Series1->XValues->Order = loAscending;
		   float xmn=SDIAppForm->Series1->MinXValue();
int trt, roh;
	   trt=AnsiPos(".TRT",AnsiUpperCase(nomefile));
	   roh=AnsiPos(".ROH",AnsiUpperCase(nomefile));
  ...
  do
   ...
   {
   if ((OKBottomDlg->CheckBox7->Checked==true) & (wave < 1500)) wave=wave*10;
   if ((wave <= atof(AnsiString(OKBottomDlg->Edit4->Text).c_str())) && (wave >= atof(AnsiString(OKBottomDlg->Edit3->Text).c_str())) && (wave < xmn)) SDIAppForm->Series1->AddXY(wave+atof(AnsiString(OKBottomDlg->Edit23->Text).c_str()),lips,' ',clBlack);
   }
   }
   while (!feof(stream));
  fclose(stream);
...
void __fastcall TSDIAppForm::ToolButton2Click(TObject *Sender)
{
  float findfirstvalue(AnsiString nomefile);
 void appfile(AnsiString nomefile);
  void appfileb(AnsiString nomefile);
 ...
 void appfile2(AnsiString nomefile);
  ...
  {
  SaveDialog->FileName = OpenDialog->FileName;
  if (SDIAppForm->OpenDialog->Files->Count==1) {
	 float fv0=findfirstvalue(OpenDialog->FileName);

		   float xmn=SDIAppForm->Series1->MinXValue();
		if (fv0 >= xmn) {
	appfile(OpenDialog->FileName);
	  fp=(ExtractFilePath(OpenDialog->FileName));
		}
		else  {
			appfileb(OpenDialog->FileName);
	  fp=(ExtractFilePath(OpenDialog->FileName));
        }

  }
   else if (SDIAppForm->OpenDialog->Files->Count==2) {
   AnsiString fvv1= AnsiString(OpenDialog->Files->Strings[0]);
	 AnsiString fvv2= AnsiString(OpenDialog->Files->Strings[1]);
	  AnsiString fvv3;
	float fv1=findfirstvalue(fvv1);
	 float fv2=findfirstvalue(fvv2);
  ...
  if (fv1 > fv2) {
		...
		if (Form13->StringGrid1->Cells[1][0]=="")
		{
			openfile(fvv1.c_str());
			appfile(fvv2.c_str());
		}
  ...
  SaveDialog->FileName = AnsiString(fvv1.c_str());
	int y;
   for (y=1;y<Form13->StringGrid1->RowCount;y++)
	 {
	 if (Form13->StringGrid1->Cells[1][y]=="")
	{
	Form13->StringGrid1->Cells[1][y]=AnsiString(fvv1.c_str());
	Form13->StringGrid1->Refresh();
	break;
	}
	  }
  ...
  compfile(fvv1.c_str());
   ...
   {
	   ShowMessage("Sorry, no more than 2 files can be concatenated.");
...
void __fastcall TSDIAppForm::OpenClick(TObject *Sender)
{
void openfile(AnsiString nomefile);
void compfile(AnsiString nomefile);
  ...
  if (I==0) {
	SDIAppForm->SaveDialog->FileName = AnsiString(OpenDialog->Files->Strings[I].c_str());
   fp=ExtractFilePath(AnsiString(OpenDialog->Files->Strings[I].c_str()));
  SDIAppForm->Edit14->Text=fp;
	 openfile(AnsiString(OpenDialog->Files->Strings[I]).c_str());
  }
   else
   {
	 SaveDialog->FileName = AnsiString(OpenDialog->Files->Strings[I]).c_str();
...
{
SDIAppForm->OpenClick(Sender);
}
//---------------------------------------------------------------------------

void __fastcall TSDIAppForm::B1Click(TObject *Sender)
{
void compfile(AnsiString nomefile);
...
void __fastcall TSDIAppForm::AutoFit1Click(TObject *Sender)
{
...
void __fastcall TSDIAppForm::Ba1Click(TObject *Sender)
{
Form36->Memo1->Clear();
Form36->Show();
}
