...
int jcount=0;
  for (int i=1; i < Form20->StringGrid1->RowCount; i++)
{
if (Form20->StringGrid1->Cells[0][i] != "X" && atof(Form20->StringGrid1->Cells[4][i].c_str()) !=0.0)
{
cnt++;
 tot+=atof(Form20->StringGrid1->Cells[4][i].c_str());
}
}
if (cnt !=0)
{
...
char sqls[300];
 char sname[10];
 sprintf(sqls,"SELECT DISTINCT specie FROM %s WHERE aki > %f AND inte > 0.0  ORDER BY specie",ExtractFileName(Form6->Table1->TableName),soglia);
