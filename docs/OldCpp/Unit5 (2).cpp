...
sp=sp.SubString(1,lg-1);
  ...
  {
  case 1:
   sprintf(lab,"%s I",Form6->Edit1->Text.c_str());
   break;
  case 2:
   sprintf(lab,"%s II",Form6->Edit1->Text.c_str());
  break;
  }
Form6->Show();
...
char sqls[300];
 ...
 sprintf(sqls,"SELECT * FROM %s where wavelen>(%s - 0.04) and wavelen < (%s + 0.04) and ion=%s", sp.c_str(),Form5->Edit1->Text.c_str(),Form5->Edit1->Text.c_str(),Form5->Edit3->Text.c_str());
      ...
      TShiftState Shift)
{
 ...
 sprintf(sqls,"SELECT * FROM %s where wavelen>(%s - 0.04) and wavelen < (%s + 0.04) and ion=%s", Form5->Edit2->Text.c_str(),Form5->Edit1->Text.c_str(),Form5->Edit1->Text.c_str(),Form5->Edit3->Text.c_str());
  Form5->Query2->SQL->Clear();
 Form5->Query2->SQL->Add(sqls);
  Form5->Query2->Open();
}
