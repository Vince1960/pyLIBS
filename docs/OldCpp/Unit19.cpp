...
delta=atof(AnsiString(OKBottomDlg->Edit8->Text).c_str());
 if (atof(AnsiString(ComboBox1->Text).c_str()) > 0)
{
 minax=atof(AnsiString(ComboBox1->Text).c_str())-delta;
  maxax=atof(AnsiString(ComboBox1->Text).c_str())+delta;
